...
def swine_align(player_score, opponent_score):
    ...
    if opponent_score >= 10 and opponent_score>0:
        return True
    return False
    # END PROBLEM 4a
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0<goal and score1<goal):
        t = take_turn(strat(who)(score(who),score(other(who))), score(other(who)), dice)
        if who:
            score1 += t
        else:
            score0 += t
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def calc_new_score(score0,score1):
        nonlocal who
        nonlocal last_score
        if who:
            return score1 - last_score, score1
        else:
            return score0 - last_score, score0
    
    def ann_hi(score0,score1):
        c, score = calc_new_score(score0,score1)
        nonlocal running_high
        nonlocal who
        new_high = running_high
        if c > running_high:
            print("%d point(s)! The most yet for Player %d" %(c,who))
            new_high = c
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if pts>=cutoff or extra_turn(score + pts, opponent_score):
        return 0
    return num_rolls  # Replace this statement
    # END PROBLEM 11
